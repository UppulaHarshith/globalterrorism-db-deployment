...
global logger
logger = get_logger(mode='cloud', project_name=PROJECT_ID, log_name='attack_pipeline')
...
def pipeline():
    try:
        ...
        # datapreprocessing
        attack_df = attack_df.replace(r'^\s*$', np.nan, regex=True)
        attack_df = attack_df.replace('?','')
        ...
        attack_df['UPDATED_ON'] = datetime.datetime.today()

        # create the table in bigquery
        if (table_exists_value == False):
            client.load_table_from_dataframe(attack_df, table_id)
            print("Data has been loaded to a new table!")
            logger.info("Data has been loaded to a new table!")
        else:
            client = bigquery.Client()
            sql_query = """select attackid from {}.{}.{}""".format(PROJECT_ID, DATASET_ID, TABLE_NAME) # find the max date from the bq table
            bq_df = client.query(sql_query).to_dataframe()
            attack_merge = attack_df.merge(bq_df.drop_duplicates(), on=['attackid'], 
                   how='left', indicator=True)
            attack_merge = attack_merge[attack_merge['_merge'] == 'left_only']
            client.load_table_from_dataframe(attack_merge, table_id)
