...
global logger
logger = get_logger(mode='cloud', project_name=PROJECT_ID, log_name='region_pipeline')
...
def pipeline():
    try:
        ...
        # datapreprocessing
        region_df = region_df.replace(r'^\s*$', np.nan, regex=True)
        region_df = region_df.replace('?','')
        ...
        region_df['UPDATED_ON'] = datetime.datetime.today()

        # create the table in bigquery
        if (table_exists_value == False):
            client.load_table_from_dataframe(region_df, table_id)
            print("Data has been loaded to a new table!")
            logger.info("Data has been loaded to a new table!")
        else:
            client = bigquery.Client()
            sql_query = """select regionid from {}.{}.{}""".format(PROJECT_ID, DATASET_ID, TABLE_NAME) # find the max date from the bq table
            bq_df = client.query(sql_query).to_dataframe()
            region_merge = region_df.merge(bq_df.drop_duplicates(), on=['regionid'], 
                   how='left', indicator=True)
            region_merge = region_merge[region_merge['_merge'] == 'left_only']
            client.load_table_from_dataframe(region_merge, table_id)
