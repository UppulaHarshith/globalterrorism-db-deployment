...
country_fact_ingestion = DAG(  
    dag_id = 'country_pipeline',
...
START_OP = DummyOperator(task_id="START",
                         dag=country_fact_ingestion)




#The following operator applies transformation.sql script and pushes the data to a validation table
country_data_pipeline = BashOperator(
    task_id='country_data_ingestion',
    wait_for_downstream=False,
    bash_command='python3 /home/airflow/gcs/dags/country_pipeline.py',
    dag=country_fact_ingestion)


# validation_data_ingest_dag_pipeline
START_OP.set_downstream(country_data_pipeline)