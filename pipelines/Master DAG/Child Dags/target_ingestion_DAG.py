...
target_fact_ingestion = DAG(  
    dag_id = 'target_pipeline',
...
START_OP = DummyOperator(task_id="START",
                         dag=target_fact_ingestion)




#The following operator applies transformation.sql script and pushes the data to a validation table
target_data_pipeline = BashOperator(
    task_id='target_data_ingestion',
    wait_for_downstream=False,
    bash_command='python3 /home/airflow/gcs/dags/target_pipeline.py',
    dag=target_fact_ingestion)


# validation_data_ingest_dag_pipeline
START_OP.set_downstream(target_data_pipeline)